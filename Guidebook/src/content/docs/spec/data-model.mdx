...
3つの条件を **AND で全部** 並べる必要がある。

```sql
CHECK (
  played_on GLOB '[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]'  -- 形式
  AND date(played_on) IS NOT NULL                              -- NULL比較の穴を塞ぐ
  AND date(played_on) = played_on                              -- 実在日付か
)
```

| 条件 | これだけだと通ってしまうもの |
|---|---|
| `GLOB` のみ | `'2026-13-99'` `'2026-02-30'`（形式は合っているが実在しない） |
| `date(x) = x` のみ | `'banana'` `'2026-13-99'` `'2026-8-6'` |
| 上記2つ（`IS NOT NULL` なし） | `'2026-13-99'` `'2026-00-10'` |

<Aside type="caution" title="date() は不正な入力に NULL を返す — CHECK は NULL を「合格」とみなす">
`date('banana')` も `date('2026-13-99')` も **NULL** を返す。SQLite の `CHECK` は式が **偽のときだけ** 拒否し、**NULL のときは通す**。つまり `CHECK (played_on = date(played_on))` は「日付として解釈できない値」を**素通りさせる**。`AND date(played_on) IS NOT NULL` を挟んで初めて塞がる。

なお `date('2026-02-30')` は NULL ではなく **`'2026-03-02'` に正規化**されて返る。この場合は `= played_on` が偽になるので弾ける。壊れ方が入力によって違うので、3条件をすべて並べるのが確実。
</Aside>

15パターンで検証済み（`2024-02-29` は通り `2025-02-29` は弾く、`UPDATE` でも効く）。
