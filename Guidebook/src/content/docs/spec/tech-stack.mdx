...
flowchart LR
  ...
  GH -->|GitHub Actions<br/>wrangler deploy| W
  ...
  Dev -->|wrangler d1<br/>migrations / execute / export| D1
```
...
```bash
wrangler d1 create majan --location apac
```
...
type Bindings = {
  ...
  WRITE_PASSCODE: string;  // wrangler secret put で登録
};
...
- majan/
  ...
  - migrations/ 0001_init.sql, 0002_....sql（wrangler d1 migrations が参照）
  - db/
    - seed.sql 初期データ（メンバー10人・リーグ・チーム）
  - content/
    - rules.md 対局ルール
  ...
  "d1_databases": [
    {
      "binding": "DB",
      "database_name": "majan",
      "database_id": "<wrangler d1 create majan で発行されたID>"
    }
  ]
}
```
