...
```bash
wrangler d1 export majan --remote --output=majan-before-reset.sql
```
...
```bash
wrangler d1 execute majan --remote --command "SELECT (SELECT COUNT(*) FROM games) AS games, (SELECT COUNT(*) FROM game_results) AS results, (SELECT COUNT(*) FROM members) AS members;"
```
...
```bash
wrangler d1 execute majan --remote --file=db/reset-games.sql
```
...
```bash
wrangler d1 execute majan --remote --command "SELECT (SELECT COUNT(*) FROM games) AS games, (SELECT COUNT(*) FROM game_results) AS results, (SELECT COUNT(*) FROM members) AS members, (SELECT COUNT(*) FROM league_members) AS roster, (SELECT name FROM leagues WHERE id = 1) AS league;"
```
