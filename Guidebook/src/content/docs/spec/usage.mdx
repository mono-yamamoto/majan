...
```bash
bash scripts/backup-games.sh
```

`~/majan-games-backup-YYYYMMDD-HHMMSS.sql` に、**流し直せる `INSERT` 文**として書き出す。最後に **DB の件数と書き出した件数が一致するか**を確かめ、食い違ったら失敗で止まる。

```
書き出し: /Users/xxx/majan-games-backup-20260827-181500.sql
  games        13 件
  game_results 52 件
  DB 上の件数   13/52
  ★ 一致
...
```bash
npx wrangler d1 execute majan --remote --file=~/majan-games-backup-20260827-181500.sql
```

<Aside type="caution" title="`wrangler d1 export` は使えない">
本来はこれで丸ごと書き出せるはずだが、**`wrangler login` の OAuth トークンでは D1 の export API が落ちる**。

```
npx wrangler d1 export majan --remote --output=backup.sql
→ Authentication error [code: 10000]
```

**`d1 (write)` 権限も Super Administrator も付いていても落ちる。** `d1 execute` は同じトークンで通るので、権限の問題ではなく **export のエンドポイントが OAuth トークンを受け付けない**という話。

API トークンを別に作れば `export` も使えるが、**リセットで消すのは `games` と `game_results` だけ**なので、その2つを `INSERT` 文にしておけば足りる。**トークンを増やさない方を選んだ。**
</Aside>

<Aside type="note" title="このバックアップに実名は入らない">
書き出すのは `games` と `game_results` だけ。**`members` は書き出さない**（リセットで消さないので、バックアップする必要がない）。`game_results` が持つのは `member_id` だけ。

**丸ごと export した場合は実名が入る**ので、そのときはリポジトリの外に置くこと。
</Aside>
