...
<Steps>
1. `db/seed.sql` を **`db/seed.local.sql` にコピーして** 10人の名前・チーム分けを書く（→ 下の Aside）
2. マイグレーションを適用（`wrangler d1 migrations apply majan --remote`）してテーブルを作る
3. `seed.local.sql` を流し込む
4. `WRITE_PASSCODE` を設定し、入力係にだけ共有する
5. アプリURLを全員に共有
</Steps>

<Aside type="caution" title="実名は seed.local.sql に書く。seed.sql は編集しない">
`db/seed.sql` は **プレースホルダ名（山田・佐藤…）のテンプレート** で、リポジトリにコミットされている。**ここに実名を書いてはいけない。**

```bash
cp db/seed.sql db/seed.local.sql   # これを編集して実名を入れる
```

`db/seed.local.sql` は `.gitignore` 済みなので、**メンバー10人の実名が git 履歴に残らない**。リポジトリや Cloudflare アカウントの管理が将来変わっても、身内の実名が付いて回ることがなくなる。

流し込むのは常に `seed.local.sql` の方。
</Aside>

<Tabs>
  <TabItem label="seed.local.sql">
```sql title="db/seed.local.sql"（db/seed.sql をコピーして実名に置き換える）
